...
> **Status (2026-07-04)**: Decision made to **unify this into the MoltsPay SDK** as a third payment mode alongside crypto (per-transaction signing) and fiat QR rails (Alipay/WeChat, per-transaction scan-to-pay). This document was migrated from a standalone design (`webchatpay-design.md`) originally written for an **independent WebchatPay service (port 4402) — that route is now abandoned**. The data model, API semantics, and security design below remain valid, but the host becomes `MoltsPayServer` / `MoltsPayClient`; the final section ("Relationship to MoltsPay") describing the "standalone short-term build" no longer applies. Concrete SDK integration design (x402 scheme, facilitator interface, top-up reuse of existing rails) is still to be written.
