...
Each order's `out_trade_no` is generated by the facilitator (unique). The `code_url` looks like `weixin://wxpay/bizpayurl?pr=xxxxxxx` — render it verbatim into a QR; **do not** post-process it.
