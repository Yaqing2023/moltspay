...
#### Deferred Payment System (Credit-based Pay Later)

Complete deferred payment infrastructure for trusted Agent-to-Agent transactions.

**DeferredPaymentManager** - Core manager for credit accounts and deferred payments
```typescript
...
// Later, record settlement when buyer pays
...
- **Account Suspension** - Automatically suspend overdue accounts

**Storage Options:**
- `MemoryDeferredStore` - In-memory (testing)
- `JsonDeferredStore` - File-based persistence (single process)
- Custom stores via `DeferredPaymentStore` interface
...
- `[status:settlement_received payment=dp_xxx tx=0x... amount=3.99 USDC]`
- `[status:account_settled id=ca_xxx tx=0x... amount=10 USDC]`
- `[status:payment_overdue payment=dp_xxx amount=3.99 USDC]`

### New Exports

```typescript
// Manager
export { DeferredPaymentManager } from 'moltspay';

// Stores
export { MemoryDeferredStore, JsonDeferredStore } from 'moltspay';

// Templates
export { DeferredSellerTemplates, DeferredBuyerTemplates, DeferredStatusMarkers } from 'moltspay';
